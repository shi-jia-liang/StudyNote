...
&emsp;&emsp;1\*1的卷积层是用于降维或者升维的。输入RGB图像，其维度为3。经过一层卷积层有256个卷积核之后，其维度变为256。1\*1的卷积层在不改变特征矩阵的矩阵大小的情况下，对矩阵进行降维或者升维操作。
...
&emsp;&emsp;通过堆叠多个3\*3的卷积核来替代大尺度卷积核（减少所需参数）,通过堆叠两个3\*3的卷积核来替代5\*5的卷积核，三个3\*3的卷积核来替代7\*7的卷积核，具有相同的感受野。
&emsp;&emsp;(例如，5\*5\*3的图像，使用两个3\*3\*3的卷积核，保证其维度相同，最终得到5\*5\*2的结果，在实际计算过程中，会用0填充边界，其次在由于使用两个卷积核则会得到两个维度)通过设定多个卷积核，可以提取多种特征，增加模型的表达能力，增强模型的鲁棒性。
...
3. 将每个特征矩阵通过ROI　pooling层缩放倒7\*7的特征图，接着 将特征图展平通过一系列全连接层得到预测结果
...
#### 转置卷积
&emsp;&emsp;转置卷积不是卷积的逆运算，转置卷积也是卷积。它能将2\*2矩阵变换成4\*4矩阵。
